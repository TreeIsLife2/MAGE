<template>
  <div id="auth" class="h-full grid pl-10 pr-10 grid-cols-2 grid-rows-1 dark:bg-gray-900">
    <div id="accounts-wrapper" class="col-start-1 row-start-1">
      <div class="account-container" v-for="(farm, index) in farms" :key="index">
        <div class="account p-4 bg-gray-200 dark:bg-gray-800 mt-1 mb-1 hover:bg-gray-300 dark:hover:bg-gray-900 select-none flex" v-if="!farm.isFarm">
          <div>{{farm.name}}</div>
          <div class="ml-auto">
    ...
    <div id="profiles-wrapper" class="hidden col-start-1 col-end-3 row-start-1 bg-white dark:bg-gray-900">
      <cp />
